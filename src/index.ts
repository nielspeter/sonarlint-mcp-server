  ...
  try {
    switch (name) {
      ...
      default:
        throw new SloopError(
          `Unknown tool: ${name}`,
          `The tool '${name}' is not recognized. Available tools: analyze_file, analyze_files, analyze_content, list_active_rules, health_check, analyze_project, apply_quick_fix`,
          false
        );
    }
  } catch (error) {
