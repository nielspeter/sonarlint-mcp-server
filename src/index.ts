  ...
  'check_quality',
  {
    ...
    inputSchema: {
      ...
      minSeverity: z
        ...
        .optional()
        .describe('Minimum severity level to include. Filters out issues below this level. Default: INFO (show all)'),
      excludeRules: stringArray
        .optional()
        .describe("List of rule IDs to exclude (e.g., ['typescript:S1135', 'javascript:S125'])"),
    },
  },
  async (args) => {
    try {
...
server.registerTool(
  'check_files',
  {
    ...
    inputSchema: {
      filePaths: stringArray.describe(
        "Array of file paths or glob patterns to analyze (e.g., ['/path/to/file.ts', 'src/**/*.js'])",
      ),
      basePath: z
        .string()
        .optional()
        .describe(
          "Project root directory for resolving relative paths and globs (e.g., '/Users/me/project'). Required when filePaths contains relative paths.",
        ),
  ...
  'check_code',
  {
    ...
    inputSchema: {
      ...
      language: z
        ...
        .describe('Programming language of the content'),
  ...
  'list_rules',
  {
    ...
    inputSchema: {
      language: z
        .enum(['javascript', 'typescript', 'python', 'java', 'go', 'php', 'ruby'])
        .optional()
        .describe('Filter rules by language (optional)'),
    },
  },
  async (args) => {
    try {
...
async function main() {
  ...
  console.error(
    '[MCP] Tools: check_quality, check_files, check_code, list_rules, fix_issue, fix_all_issues, health_check',
  );
